    ...
    if ( token.startsWith( 'Bearer ' ) ) {
        ...
        if( err ) {
          ...
          if (decoded.role != 'admin'){
            return res.json({
              success: false, 
              message: 'El usuario no tiene acceso a este recurso'
            })
          }else{
            next();
          }
...
let checkAdminEmployee = ( req, res, next ) => {
  ...
  if( token ) {
    ...
    if ( token.startsWith( 'Bearer ' ) ) {
        ...
        if( err ) {
          ...
          req.decoded = decoded;
          if (decoded.role != 'admin' && decoded.role != 'empleado'){
            return res.json({
              success: false, 
              message: 'El usuario no tiene acceso a este recurso'
            })
...
module.exports = {
  ...
  checkAdminEmployee: checkAdminEmployee
